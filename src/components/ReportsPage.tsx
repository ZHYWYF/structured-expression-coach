...
import type { RecordingTask, Report, ScoreDimension } from "../core/types";
...
export function ReportsPage({ controller }: { controller: WorkspaceController }) {
  ...
  const uploadAudio = async (file?: File) => {
    ...
    const task: RecordingTask = { id: `recording-${crypto.randomUUID()}`, sessionId: session.id, title: file.name, sourceFileName: file.name, sourceMimeType: file.type, status: "queued", provider: controller.preferences.transcriptionProvider, progress: 0, reportStatus: "not-generated", createdAt: now, updatedAt: now };
    try {
  ...
  const generateReport = async () => {
    ...
    try {
      const content = await requestChatCompletion(controller.preferences.aiProvider, [
        { role: "system", content: "你是结构化表达教练。只分析用户提供的逐字稿，只返回 JSON，不虚构内容。" },
        { role: "user", content: `逐字稿：\n${selectedTask.transcript}\n\n返回 {"title":"","overallScore":0,"dimensions":[{"key":"structure","label":"结构","score":0,"summary":""},{"key":"clarity","label":"清晰度","score":0,"summary":""},{"key":"evidence","label":"证据","score":0,"summary":""},{"key":"brevity","label":"简洁度","score":0,"summary":""},{"key":"confidence","label":"自信度","score":0,"summary":""}],"strengths":[],"improvements":[],"actionItems":[]}。` },
      ]);
  ...
  return (
    <div className="page reports-page">
      <PageHeader eyebrow="录音与报告" title="上传真实录音，保留可校对的逐字稿" description="本地与在线转写明确分开；任何失败都不会生成演示文本。" action={<><input ref={inputRef} className="visually-hidden" type="file" accept="audio/*,.wav,.mp3,.m4a,.aac,.flac,.ogg" onChange={(event) => void uploadAudio(event.target.files?.[0])} /><PrimaryButton onClick={() => inputRef.current?.click()}><FileUp size={16} /> 上传录音</PrimaryButton></>} />
      <div className="transcription-mode"><span>本次默认使用</span><button type="button" className={controller.preferences.transcriptionProvider === "local" ? "active" : ""} onClick={() => controller.updatePreferences({ transcriptionProvider: "local" })}>本地转写</button><button type="button" className={controller.preferences.transcriptionProvider === "online" ? "active" : ""} onClick={() => controller.updatePreferences({ transcriptionProvider: "online" })}>在线高精度</button><small>{controller.preferences.transcriptionProvider === "online" ? "音频将发送到你配置的在线服务" : "音频不离开当前设备"}</small></div>
