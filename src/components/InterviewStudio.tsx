...
export function InterviewStudio({ controller }: { controller: WorkspaceController }) {
  ...
  const generateQuestions = async () => {
    ...
    try {
      const content = await requestChatCompletion(controller.preferences.aiProvider, [
        { role: "system", content: "你是严谨的中文面试教练。只能依据用户提供的 JD 和简历生成问题，不得虚构候选人经历。只返回 JSON。" },
        { role: "user", content: `JD：\n${selectedSession.jobDescription.content}\n\n简历：\n${selectedSession.resume.content}\n\n生成 6 道覆盖核心能力、经历深挖和压力追问的问题。返回 {"questions":[{"tag":"","text":"","suggestedMinutes":3,"jdEvidence":"JD原文片段"}]}。` },
      ]);
  ...
  const analyzeAnswer = async () => {
    ...
    try {
      const content = await requestChatCompletion(controller.preferences.aiProvider, [
        { role: "system", content: "你是中文面试教练。严格依据 JD、简历和回答分析，不得补造材料中不存在的事实。只返回 JSON。" },
        { role: "user", content: `JD：${selectedSession.jobDescription.content}\n简历：${selectedSession.resume.content}\n问题：${question.text}\n回答：${answer}\n返回 {"structure":"","jdMatch":"","resumeConsistency":"","evidenceStrength":"","overallSuggestion":""}，建议只指出如何局部改善，不要重写整段。` },
      ]);
