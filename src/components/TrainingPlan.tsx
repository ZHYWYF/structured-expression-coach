...
export function TrainingPlan({ controller }: { controller: WorkspaceController }) {
  ...
  const baselineReports = controller.sessions.filter((session) => session.scenarioId === editing?.scenarioId && session.report).map((session) => session.report!).sort((a, b) => b.generatedAt.localeCompare(a.generatedAt));
