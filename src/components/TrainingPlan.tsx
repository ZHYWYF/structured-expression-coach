...
function blankPlan(scenarioId: string): TrainingPlanModel {
  ...
  return {
    ...
    startDate: iso.slice(0, 10), endDate: addDays(now, 13), focusAreas: [], tasks: [], createdAt: iso, updatedAt: iso,
  };
}
...
export function TrainingPlan({ controller }: { controller: WorkspaceController }) {
  ...
  const averageScore = reports.length ? Math.round(reports.reduce((sum, report) => sum + report.overallScore, 0) / reports.length) : 0;
  ...
  const savePlan = () => {
    if (!editing?.title.trim() || !editing.goals.length || !editing.tasks.length || activeConflict) return;
      ...
      <div className="page training-page">
        ...
        <section className="plan-editor">
          ...
          <div className="form-section"><div className="form-section-title"><span>02</span><div><h2>当前水平与场景</h2><p>支持自评；后续完成基线练习后可以修正。</p></div></div>
            ...
            <div className="quick-actions"><button type="button" className={editing.levelSource === "self-assessment" ? "selected" : ""} onClick={() => setEditing({ ...editing, levelSource: "self-assessment" })}>使用自评</button><button type="button" className={editing.levelSource === "baseline" ? "selected" : ""} disabled={!reports.length} onClick={() => { const level = averageScore >= 85 ? "advanced" : averageScore >= 70 ? "intermediate" : "beginner"; setEditing({ ...editing, levelSource: "baseline", currentLevel: level }); }}>使用最近报告评估{reports.length ? `（${averageScore} 分）` : "（暂无报告）"}</button></div>
          </div>
          ...
          <div className="plan-editor-actions"><button className="button-secondary" type="button" onClick={() => setEditing(null)}>取消</button><PrimaryButton onClick={savePlan} disabled={!editing.title.trim() || !editing.goals.length || !editing.tasks.length || Boolean(activeConflict)}><Save size={15} /> 保存计划</PrimaryButton></div>
        </section>
      </div>
    );
  }
  ...
  return (
    <div className="page training-page">
      ...
      {plan ? <><section className="training-overview"><div className="plan-summary"><p className="eyebrow">{statusLabels[plan.status]}</p><h2>{plan.title}</h2><p>{plan.description || plan.goals.join(" · ")}</p><div className="plan-progress"><span style={{ width: `${tasks.length ? (completed / tasks.length) * 100 : 0}%` }} /></div><small>已完成 {completed}/{tasks.length} 项</small><div className="plan-summary-actions"><button type="button" onClick={() => setEditing(structuredClone(plan))}><Edit3 size={14} /> 编辑</button>{plan.status === "completed" || plan.status === "archived" ? <button type="button" onClick={() => setEditing({ ...structuredClone(plan), id: newId("plan"), title: `${plan.title}（新）`, status: "draft", tasks: plan.tasks.map((task) => ({ ...task, id: newId("task"), status: "todo", completedAt: undefined })), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() })}><Plus size={14} /> 复制为新计划</button> : <button type="button" onClick={() => controller.upsertTrainingPlan({ ...plan, status: plan.status === "active" ? "paused" : "active", updatedAt: new Date().toISOString() })}>{plan.status === "active" ? <Pause size={14} /> : <Play size={14} />}{plan.status === "active" ? "暂停" : plan.status === "paused" ? "继续" : "开始"}</button>}<button type="button" onClick={() => { if (window.confirm(`删除训练计划“${plan.title}”？`)) controller.deleteTrainingPlan(plan.id); }}><Trash2 size={14} /> 删除</button></div></div><div className="plan-metrics"><div><span><Flame size={18} /></span><strong>{completed} 项</strong><small>本计划已完成</small></div><div><span><Target size={18} /></span><strong>{averageScore || "--"}</strong><small>关联练习平均分</small></div></div></section><section className="task-section"><div className="section-heading compact"><div><p className="eyebrow">训练安排</p><h2>计划任务</h2></div><span>共 {tasks.reduce((sum, task) => sum + task.targetMinutes, 0)} 分钟</span></div><div className="task-list">{tasks.map((task) => { const done = task.status === "done"; return <button className={done ? "task-row done" : "task-row"} type="button" key={task.id} onClick={() => controller.updateTrainingTask(plan.id, task.id, done ? "todo" : "done")}><span className="task-check">{done ? <Check size={15} /> : <Circle size={17} />}</span><span className="task-day">{task.targetMinutes} 分钟</span><span className="task-copy"><strong>{task.title}</strong><small>{task.description}</small></span></button>; })}</div></section></> : <section className="empty-state"><Target size={30} /><h2>还没有训练计划</h2><p>设置目标、当前水平、周期和任务后，首页会展示真实训练进度。</p><PrimaryButton onClick={startNew}><Plus size={15} /> 创建第一个计划</PrimaryButton></section>}
    </div>
  );
}
